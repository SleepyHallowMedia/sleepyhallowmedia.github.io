/* Sleepy Hallow Media — Magazine Script (v6.2)
   Purpose: modernized, bold-but-modest cover lead + immersive article polish.
   Keeps original behaviors: manifest-driven rendering, search, tags, a11y, image hints.
*/
'use strict';
...
/* Lead card: overlay + vertical masthead + sticker */
...
async function initArticlePage(){
  ...
  if(warmed){ try{ const parsed=parseFrontmatter(warmed); renderArticle(content, file, parsed.meta, parsed.body); buildShareLinks(parsed.meta.Title||file); return; }catch{} }
  try{ const parsed=await loadNewsletter(file); renderArticle(content, file, parsed.meta, parsed.body); buildShareLinks(parsed.meta.Title||file); }catch(e){ console.error(e); content.innerHTML=`<div class="muted">Could not load this article.</div>`; content.removeAttribute('aria-busy'); }
}
...
function initCoverParallax(){
  ...
  const onMove = (e) => {
    ...
    tx = (cx / r.width - .5) * 4;  // small, modest shift
    ty = (cy / r.height - .5) * 4;
